...
MongoDB. Используется для:
 ...
 - ведения пользовательского баланса в токенах
 - сохранение использованного пользователем объема токенов за все время 

Ее использование **не обязательно**, будет использоваться только при указании `MONGO_HOST_PORT` в `.env`.  
